...
export async function createAccountAction(
  ...
  try {
    ...
    if (clash) {
      return {
        ok: false,
        error: `Account ${code} already exists (${clash.name}).`,
      };
    }
 ...
 * in the ledger. Deactivating hides them from pickers while leaving
 * history intact.
 */
export async function toggleAccountAction(
  ...
  try {
    ...
    if (account.active) {
      ...
      const dept = await db.department.count({
        where: { orgId: org.id, cashAccountCode: account.code },
      });
      if (dept > 0) {
        return {
          ok: false,
          error: `${account.code} is the cash account for ${dept} department(s).`,
        };
      }
