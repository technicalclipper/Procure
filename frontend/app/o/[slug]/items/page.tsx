    ...
    <div className="mx-auto max-w-6xl px-8 py-8">
      ...
      {accounts.length === 0 && (
        <div className="mb-6 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] text-amber-900">
          ...
          <Link href={`/o/${slug}/accounts`} className="underline">
            chart of accounts
          </Link>{" "}
      ...
      {canManage && (
        <div className="mb-6">
          <AddItem slug={slug} accounts={accounts} />
        </div>
      )}
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-[12px] text-slate-500">
            Items are what a requester picks from — &ldquo;Cloud
            compute&rdquo;, not account 5000. Their default account does the
            GL coding.
          </p>
        </div>
      ) : (
