...
const GROUPS: { type: AccountType; label: string; note?: string }[] = [
  { type: AccountType.ASSET, label: "Assets", note: "One cash account per department wallet" },
  { type: AccountType.LIABILITY, label: "Liabilities" },
  { type: AccountType.EQUITY, label: "Equity" },
  { type: AccountType.INCOME, label: "Income" },
  { type: AccountType.EXPENSE, label: "Expenses", note: "Items post here automatically via their default account" },
];
      ...
      <header className="mb-7 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1 text-[13px] text-slate-500">
            Every movement in the procure-to-pay cycle posts a balanced journal
            entry against these accounts.
          </p>
        </div>
        <div className="shrink-0 text-right">
          ...
          <div className="text-[11px] text-slate-400">
            active{counts.total !== counts.active && ` of ${counts.total}`}
          </div>
        </div>
      </header>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="w-28 px-4 py-3 text-right align-top">
                            <ToggleAccount id={a.id} active={a.active} />
                          </td>
                        </tr>
                      );
                    })}
